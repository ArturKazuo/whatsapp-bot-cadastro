...
const LOG_FILE = `./${SESSION_NAME}.csv`;
...
async function sendText(phone, text) {
  ...
  try {
    await bot.sendTextNew(phone, text);
    await addlog(phone, text);
  } catch (error) {
	await addlog(phone, `[ERROR] ${JSON.stringify(error)}`);
    console.log(error);
	throw error;
...
function addlog(phone, text) {

  const date = (new Date()).toISOString();
  const csv = `${date};${phone};${text}\n`;
...
app.get('/log', function (req, res) {
  ...
  try {
    const dataRaw = fs.readFileSync(LOG_FILE, 'utf8');
